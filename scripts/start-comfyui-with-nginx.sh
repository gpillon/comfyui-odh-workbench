...
if [ "$API_MODE" = "true" ]; then
    # API mode flags
    mkdir -p /tmp/comfyui-input
    mkdir -p /tmp/comfyui-output
    mkdir -p /tmp/comfyui-user
    ADDITIONAL_FLAGS="--base-directory /mnt/models --database-url sqlite:///:memory: --input-directory /tmp/comfyui-input --output-directory /tmp/comfyui-output --user-directory /tmp/comfyui-user"
else
